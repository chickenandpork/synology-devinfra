...
from pydhcplib3.dhcp_network import *
...
if len(dockervars) > 0:
    ...
    summarymessages.append("NOTE: using docker vars")
...
if len(redisvars) > 0:
    ...
    logging.debug(
        "Using Redis caching for clients to host %s on port %s" % (redishost, redisport)
    )
    summarymessages.append(f"NOTE: Redis caching on {redishost}:{redisport}")
else:
    useredis = False
...
parser = argparse.ArgumentParser(prog="bsdpserver")
parser.add_argument(
    "-p",
    "--path",
    action=EnvDefault,
    envvar="BSDPY_NBI_PATH",
    help="The path to serve NBIs from. [default: /nbi]",
)
parser.add_argument(
    "-r",
    "--proto",
    ...
    default="http",
    envvar="BSDPY_PROTO",
    help="The protocol to serve NBIs with. [default: http]",
)
parser.add_argument(
    "-i",
    "--iface",
    action=EnvDefault,
    envvar="BSDPY_IFACE",
    help="The interface to bind to. [default: eth0]",
)
arguments = vars(parser.parse_args(sys.argv[1:]))  # convert Namespace → dict
...
# serverip seems to be obscured by some obscure python scoping
serverip_list = []
...
try:
    if "BSDPY_IP" in dockervars:
        ...
        serverip_list = list(serverip)
        logging.debug("Found $BSDPY_IP - using custom external IP %s" % externalip)
        ...
        serverip_list = list(serverip)
        logging.debug(
            "No BSDPY_IP env var found, using IP from %s interface" % serverinterface
        )
    summarymessages.append(f"NOTE: listening on {serverinterface}")
    if not useapiurl:  # BSDPY_API_URL not in env
        if "BSDPY_NBI_URL" in dockervars:
            ...
            try:
                ...
                logging.debug(
                    "Resolving BSDPY_NBI_URL to IP - %s -> %s"
            ...
            if "http" in bootproto:
                ...
                logging.debug(
                    "Found BSDPY_NBI_URL - using HTTP basedmgpath %s" % basedmgpath
                )

            if "nfs" in bootproto:
                basedmgpath = "nfs:" + serverip_str + ":" + tftprootpath + ":"
                logging.debug(
                    "Found BSDPY_NBI_URL - using NFS basedmgpath %s" % basedmgpath
                )
            ...
            elif "nfs" in bootproto:
                basedmgpath = "nfs:" + serverip_str + ":" + tftprootpath + ":"
                nbiurl = basedmgpath
                logging.debug("Using NFS basedmgpath %s" % basedmgpath)
            else:
                logging.info(
                    f"proto '{bootproto}' contains neither http nor nfs.  Segv inbound."
                )

        summarymessages.append(f"NOTE: bootproto at {basedmgpath}")
    ...
    logging.debug(
        "Error setting serverip, serverhostname or basedmgpath: %s" % sys.exc_info()[1]
    )
    raise
    ...
    try:
        if "http" in bootproto:
            if dockervars.get("BSDPY_NBI_URL"):
                ...
                basedmgpath = "http://%s%s/" % (nbiurlhostname, nbiurl.path)
                logging.debug(
                    "Found BSDPY_NBI_URL - using basedmgpath %s" % basedmgpath
                )
            else:
                # basedmgpath = 'http://' + serverip_str + '/'
                basedmgpath = nbiurl
        ...
        logging.debug(
            "getBaseDmgPath - Error setting basedmgpath %s" % sys.exc_info()[1]
        )
...
class DhcpServer(DhcpNetwork):
    def __init__(
        ...
        if (
            "darwin" in platform or True
        ):  # FIXME: need to opt-in for reuse on non-mac platforms
...
def getNbiOptions(incoming):
    ...
    try:
        for path, dirs, files in os.walk(incoming):
            ...
            if os.path.splitext(path)[1] == ".nbi":
                ...
                if nbimageinfo["Index"] == 0:
                    logging.debug(
                        'Image "%s" Index is NULL (0), skipping!' % nbimageinfo["Name"]
                    )
                    continue
                elif nbimageinfo["IsEnabled"] is False:
                    logging.debug(
                        'Image "%s" is disabled, skipping.' % nbimageinfo["Name"]
                    )
                    continue
                else:
                    thisnbi["id"] = nbimageinfo["Index"]

                if "BootFile" in nbimageinfo:
                    logging.debug(
                        f"line 448: Bootfile -> {find(nbimageinfo['BootFile'], path)}"
                    )
                ...
                if nbimageinfo["Type"] != "BootFileOnly":
                    if nbimageinfo.get("RootPath"):
                        ...
                        if os.path.islink(bootimage):
                            ...
                            logging.debug(
                                "Image %s RootPath is a symlink, resolving..."
                                % nbimageinfo["Name"]
                            )
                        else:
                ...
                thisnbi["enabledmacaddrs"] = [
                    mac.lower() for mac in thisnbi["enabledmacaddrs"]
                ]
        ...
        logging.debug("end of parse at 509")
    except:
        logging.debug("Unexpected error getNbiOptions: %s" % sys.exc_info()[1])
        raise

    logging.debug(f"returning options = {nbioptions}, sources = {nbisources}")
    ...
    try:
        ...
        for thisnbi in nbisources:
            ...
            if (
                ...
                and clientsysid in thisnbi["enabledsysids"]
            ):
            ...
            if not hasdupes:
                ...
                if (
                    thisnbi["enabledmacaddrs"]
                    and clientmacaddr not in thisnbi["enabledmacaddrs"]
                ):
                    logging.debug(
                        "MAC address " + clientmacaddr + " is not "
        ...
        logging.debug(
            "Unexpected error filtering image entitlements: %s" % sys.exc_info()[1]
        )
        raise
...
def doEntitlementPostProcessing(nbientitlements):
    ...
    try:
        ...
        for image in nbientitlements:
            logging.debug("This image: %s" % image)
            ...
            logging.debug(f"imageid is now {imageid}")
            ...
            imagenameslist += (
                [129, 0] + imageid + [image["length"]] + strlist(image["name"]).list()
            )

    except:
        logging.debug("Unexpected error setting default image: %s" % sys.exc_info()[1])
        raise
...
def ack(packet, msgtype, defaultnbi, basedmgpath=basedmgpath):
    ...
    try:
        ...
        clientsysid = str(strlist(packet.GetOption("vendor_class_identifier"))).split(
            "/"
        )[2]
        ...
        if useapiurl:
            ...
            apientitlements = getNbiFromApi(
                {
                    "mac_address": clientmacaddr,
                    "model_name": clientsysid,
                    "ip_address": str(clientip),
                }
            )
        ...
        if "reply_port" in bsdpoptions:
            replyport = int(
                str(
                    ...
                    + format(bsdpoptions["reply_port"][1], "x")
                ),
                16,
            )
        else:
        ...
        logging.debug(
            "Unexpected error: ack() common %s - %s" % sys.exc_info()[0],
            sys.exc_info()[1],
        )
        raise
    ...
    bsdpack.SetOption("siaddr", serverip_list)
    ...
    bsdpack.SetOption("server_identifier", serverip_list)
    ...
    if msgtype == "list":
        try:
            ...
            defaultnbioption = [7, 4, 129, 0] + [
                int(defaultnbioption[i : i + n], 16)
                for i in range(0, len(defaultnbioption), n)
            ]
            ...
            logging.debug("-=========================================-")
            logging.debug(
                "Return ACK[LIST] to %s - %s on port %s"
                % (clientmacaddr, str(clientip), str(replyport))
            )
            if hasnulldefault is False:
                logging.debug("Default boot image ID: %i" % defaultnbi)
        except:
            logging.debug("Unexpected error ack() list: %s" % sys.exc_info()[1])
            raise
    ...
    # Process BSDP[SELECT] requests
        ...
        try:
            imageid = int(
                ...
                + "%02X" % bsdpoptions["selected_boot_image"][3],
                16,
            )
        except:
            logging.debug(
                "Unexpected error ack() select: imageid %s" % sys.exc_info()[1]
            )
            raise
        ...
        for nbidict in enablednbis:
            try:
                ...
                logging.debug(
                    "Unexpected error ack() selectedimage: %s" % sys.exc_info()[1]
                )
                raise
        ...
        try:
            ...
            logging.debug(
                "Unexpected error ack() select SetOption('file'): %s"
                % sys.exc_info()[1]
            )
            raise
        try:
            ...
            logging.debug(
                "Unexpected error ack() select SetOption('root_path'): %s"
                % sys.exc_info()[1]
            )
            raise
        try:
            ...
            logging.debug(
                "Unexpected error ack() select SetOption('vendor_encapsulated_options'): %s"
                % sys.exc_info()[1]
            )
            raise
        ...
        try:
            ...
            logging.debug("-=========================================-")
            logging.debug(
                "Return ACK[SELECT] to %s - %s on port %s"
                % (clientmacaddr, str(clientip), str(replyport))
            )
            logging.debug(
                "--> TFTP URI: tftp://%s%s"
            ...
            logging.debug("--> Boot Image URI: %s" % (str(rootpath)))
        except:
            logging.debug(
                "Unexpected error ack() select print debug: %s" % sys.exc_info()[1]
            )
            raise
...
def getNbiFromApi(apiquery, names=False):
    ...
    if useredis and not names:
        cachedkey = "%s,%s,%s" % (
            apiquery["mac_address"],
            apiquery["model_name"],
            apiquery["ip_address"],
        )
        if redis.keys(cachedkey):
            logging.info(
                "Using cached data for MAC %s and model %s"
                % (apiquery["mac_address"], apiquery["model_name"])
            )
            cachedresults = {}
            ...
            logging.info(
                "Caching data for MAC %s and model %s"
                % (apiquery["mac_address"], apiquery["model_name"])
            )
                ...
                if "name" in k:
                    ...
                    if ".nbi" in v:
                        ...
                        try:
                            ...
                            logging.debug(
                                "Resolving BSDPY_NBI_URL to IP - %s -> %s"
                                % (nbiurlhostname, nbiurlhostip)
                            )
                        else:
                            nbi["dmg"] = nbiuri
                    else:
                        logging.debug(
                            "Missing or incorrect NBI URI %s, skipping %s"
                            % (v, nbi["name"])
                        )
                        break
            else:
...
def main():
    ...
    logging.debug("[======= Listen for DHCP messages to handle  =======]")
    for s in summarymessages:
        logging.debug(s)
    ...
    while True:
        ...
        except OSError as e:  # was select.error pre-PEP3151
            if e[0] != errno.EINTR:
                raise
        ...
        try:
            if packet is None:
                logging.debug(
                    "\n-==========================[-> Get Next DHCP Packet Loop <-]==========================-"
                )
            # Check to see if any vendor_encapsulated_options are present
            elif len(packet.GetOption("vendor_encapsulated_options")) > 1:
                ...
                if packet.GetOption("vendor_encapsulated_options")[2] == 1:
                    ...
                    bsdplistack, clientip, replyport = ack(
                        packet, "list", defaultnbi=defaultnbi
                    )
                ...
                #   the packet as a BSDP[SELECT] request
                    ...
                    bsdpselectack, selectackclientip, selectackreplyport = ack(
                        packet, "select", defaultnbi=defaultnbi
                    )

                    # Once we have a finished DHCP packet, send it to the client
                    server.SendDhcpPacketTo(
                        bsdpselectack, str(selectackclientip), selectackreplyport
                    )
            ...
            logging.debug(f"Exception {e}, continuing")
            pass
